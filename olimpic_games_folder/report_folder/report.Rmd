...
```
We noticed possible typo 'Footba' instead of 'Football', we can correct it:

```{r}
olimpiad_file_filtered$Sport[which(olimpiad_file_filtered$Sport == 'Footba')] <- 'Football'
```
...
olimpiad_file_filtered_winter
```

```{r}
olimpiad_file_filtered_summer <- olimpiad_file_filtered  %>% 
  filter(Season == 'Summer')
olimpiad_file_filtered_summer
```
